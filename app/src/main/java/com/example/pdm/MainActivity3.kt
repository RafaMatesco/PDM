...
class MainActivity3 : AppCompatActivity() {
    ...
    private val storage = Firebase.storage("gs://projeto2bim-b9894.appspot.com")
    ...
    fun getNomeWithIdComments():TextView {
        val database = FirebaseDatabase.getInstance()
        val lblUser = TextView(this)
        lblUser.layoutParams = LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT,
        ...
        val myRef = database.getReference("/cadastros/")
        val query = myRef.orderByChild("id").equalTo(idUserGlobal)
        query.addListenerForSingleValueEvent(object: ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot){
                for (user in snapshot.children){
                    var nome = user.child("nome").value as String
    ...
    fun addLikeComment(path: String, linearL: LinearLayout) {
        ...
        btnComment.setOnClickListener{
            val novoComment = mapOf(
                "path" to path,
                "idUsuario" to (auth.currentUser?.uid ?: String),
                "comment" to txtComment.text.toString()
            )
            myRefComments.push().setValue(novoComment).addOnCompleteListener{task ->
                if(task.isSuccessful) {
        ...
        btnLike.setOnClickListener {
            ...
            myRefLikes.push().setValue(novoLike).addOnCompleteListener{task ->
                if(task.isSuccessful) {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if(snapshot.exists()){
                    for(comment in snapshot.children){
                        ...
                        idUserGlobal = id
                        var comment = "comentário: $comentario\n"
                        lblComment.layoutParams = LinearLayout.LayoutParams(
                            LinearLayout.LayoutParams.MATCH_PARENT,
                            LinearLayout.LayoutParams.WRAP_CONTENT
                        )
                        lblComment.text = lblComment.text.toString() +  getNomeWithIdComments().text + comment
                    }
                }
            }
            override fun onCancelled(databaseError: DatabaseError) {
    ...
    fun uploadImage(): String{
        val storage = Firebase.storage("gs://projeto2bim-b9894.appspot.com")
